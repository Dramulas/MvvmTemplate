...
import '../view/moc_technical_opinion_view.dart';
import '../viewmodel/subviewmodel/moc_technical_opinion_detail_view_model.dart';
import 'build_moc_technical_opinion_acceptance_details_column.dart';

class GiveTechnicalOpinionButton extends StatefulWidget {
  final TechnicalOAModel model;
  final TechnicalOpinionDetailViewModel viewModel;
  const GiveTechnicalOpinionButton({
    Key? key,
  ...
  @override
  _GiveTechnicalOpinionButtonState createState() =>
      _GiveTechnicalOpinionButtonState();
}

class _GiveTechnicalOpinionButtonState
    extends State<GiveTechnicalOpinionButton> {
  @override
  Widget build(BuildContext context) {
    return TextButton(
      ...
      onPressed: () async {
        if (!(widget.model.techOAComment == null)) {
          ...
          if (response.statusCode == HttpStatus.ok) {
            ...
            Future.delayed(Duration(seconds: 3))
                ...
                .then((value) => Navigator.pushAndRemoveUntil(
                    context,
                    MaterialPageRoute(
                        builder: (context) => TechnicalOpinionView()),
                    (route) => route.isFirst));
          } else {
            print("hata");
            Future.delayed(Duration(seconds: 3))
